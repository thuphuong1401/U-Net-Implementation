...
# hyperparameters
loss_func_name = "Binary cross entropy loss"
learn_rate = 1e-4
num_epochs = 30
batch_size = 1
optimizer_name = "Adam"
...
with open(run_id + '/hyperparams.csv', 'w') as wfil:
	...
	wfil.write("optimizer," + str(optimizer_name) + '\n')
...
with open(run_id + '/log_file.csv', 'w') as log_fil:
    ...
    for epoch in range(0, num_epochs):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            #if i % 5 == 1:
            #    plt.imshow(outputs.detach().cpu().numpy()[0][0],cmap='gray')
            #    plt.show()
            #    plt.imshow(images.detach().cpu().numpy()[0][0],cmap='gray')
            #    plt.show()
            #    plt.imshow(labels.detach().cpu().numpy()[0],cmap='gray')
            #    plt.show()
