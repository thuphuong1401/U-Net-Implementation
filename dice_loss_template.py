...
# when using dice loss, you should have output depth of exactly 1
# sigmoid activation must be applied as the last layer in your network to ensure dice metric works properly
class UNet(nn.Module):
   def __init__(self):
      super(UNet, self).__init__()
      self.sig = nn.Sigmoid()
      #self.input_depsth=input_depth
...
class DiceLoss(Function):
	...
	def backward(ctx, grad_output):
		...
		grad_input = -2 * grad_output * (.000001 + 2 * ((target * union) - (2 * rounded * inter))) / (union * union + .000001)
